...
//static char       * file_exists = "select fname from files where fname = ?;";
static char       * file_insert = "insert into fdb_files(fname, fsize, md5_full) values(?,?,?);";
...
fdb_worker * fdb_start_workers(apr_allocator_t * apr_alloc, const int threads) {
...
static int sql_file_exists(filework *fw) {
  ...
  int l = strlen(fw->full_path);
  char *exists_sql = "select fname from fdb_files where fname = ?;";
  //printf("full_path=%s pathle=%d == %d\n", fw->full_path, fw->pathlen, l);
  sqlite3_stmt *ex_stmt;
  int rc = sqlite3_prepare_v2(fw->db, exists_sql, -1, &ex_stmt, NULL);
  if(rc != SQLITE_OK){
  ...
  rc = sqlite3_bind_text(ex_stmt,1,fw->full_path, fw->pathlen, NULL);
...
static int sql_insert_file(filework *fw) {
  ...
  sqlite3_prepare_v2(fw->db, file_insert, -1, &in_stmt,NULL);
  ...
  rc = sqlite3_bind_text(in_stmt,1,fw->full_path,  fw->pathlen, NULL);
  if(rc != SQLITE_OK ){
    printf("id=%d %s\n",rc, fw->full_path);
    assert(NULL);
  }
...
int walk_recur(char *dname, fdb_worker * worker) {
  //printf("walking=%s\n", dname);
